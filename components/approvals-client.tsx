...
import { CheckCircle2, XCircle, User, Phone, Calendar, Users, IdCard, Globe, Car, Building2, Clock, AlertTriangle, ClipboardCheck, CreditCard, Banknote, Check } from 'lucide-react';
...
export function ApprovalsClient({ 
  ...
  currentSessionId,
  userRole = 'ADMIN',
  canTrackBalance = false,
  paymentReceivers = [],
}: { 
  initialApprovals: ApprovalGroup[]; 
  currentSessionId: string;
  userRole?: string | null;
  canTrackBalance?: boolean;
  paymentReceivers?: PaymentReceiverItem[];
}) {
  const isReadOnly = userRole === 'VISUALIZER';
  const defaultReceiverId = paymentReceivers.find(r => r.isDefault)?.id || paymentReceivers[0]?.id || '';
  ...
  const [depositMethods, setDepositMethods] = useState<Record<string, 'TRANSFER' | 'CASH'>>({});
  const [depositReceiverIds, setDepositReceiverIds] = useState<Record<string, string>>({});
  ...
  const handleAction = async (groupId: string, action: 'approve' | 'reject', force = false) => {
    ...
    try {
      const depositVal = deposits[groupId] !== undefined ? deposits[groupId] : 10000;
      const methodVal = depositMethods[groupId] || 'TRANSFER';
      const receiverVal = depositReceiverIds[groupId] !== undefined ? depositReceiverIds[groupId] : defaultReceiverId;

      const bodyObj: Record<string, unknown> = action === 'approve'
        ? {
            depositAmount: depositVal,
            ...(canTrackBalance && depositVal > 0
              ? {
                  depositMethod: methodVal,
                  depositReceiverId: methodVal === 'TRANSFER' ? (receiverVal || null) : null,
                }
              : {}),
          }
        : {};
  ...
  return (
    <>
      ...
      {approvals.map((group) => {
        ...
        const currentDeposit = deposits[group.groupId] !== undefined ? deposits[group.groupId] : 10000;
        const currentMethod = depositMethods[group.groupId] || 'TRANSFER';
        const currentReceiverId = depositReceiverIds[group.groupId] !== undefined ? depositReceiverIds[group.groupId] : defaultReceiverId;
                ...
                <div className="bg-slate-50 dark:bg-slate-950/60 p-4 rounded-xl border border-slate-100 dark:border-slate-800 space-y-4">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <input 
                        type="number" 
                        className="border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 rounded-lg px-3 py-2 w-32 focus:outline-none focus:ring-2 focus:ring-sky-500 font-semibold"
                  ...
                  {/* Balance / payment tracking (only for admins who can view balance and when deposit > 0) */}
                  {canTrackBalance && currentDeposit > 0 && (
                    <div className="pt-3 border-t border-slate-200/80 dark:border-slate-800/80 space-y-3">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                        ...
                        <div className="grid grid-cols-2 gap-2 w-full sm:w-auto">
                          <button
                            type="button"
                            onClick={() => setDepositMethods(prev => ({ ...prev, [group.groupId]: 'TRANSFER' }))}
                            className={`flex items-center justify-center gap-2 px-3 py-2 rounded-lg border-2 text-xs font-semibold transition-all cursor-pointer ${
                          ...
                          >
                            ...
                            {currentMethod === 'TRANSFER' && <Check className="h-3.5 w-3.5 text-blue-500 shrink-0" />}
                          </button>
                          <button
                            type="button"
                            onClick={() => setDepositMethods(prev => ({ ...prev, [group.groupId]: 'CASH' }))}
                            className={`flex items-center justify-center gap-2 px-3 py-2 rounded-lg border-2 text-xs font-semibold transition-all cursor-pointer ${
                              currentMethod === 'CASH'
                                ? 'border-emerald-500 bg-emerald-50 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-300 shadow-xs'
                                : 'border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-400 hover:border-emerald-300'
                            }`}
                          >
                            <Banknote className="h-3.5 w-3.5 shrink-0" />
                            <span>Efectivo</span>
                            {currentMethod === 'CASH' && <Check className="h-3.5 w-3.5 text-emerald-500 shrink-0" />}
                          </button>
                        </div>
                      </div>
                      ...
                      {currentMethod === 'TRANSFER' && (
                        <div className="space-y-1.5 pt-1">
                          <label className="text-xs font-medium text-slate-600 dark:text-slate-400">
                            ¿Quién recibió la seña? (Destinatario)
                          </label>
                          {paymentReceivers.length > 0 ? (
                            <Select
                              value={currentReceiverId || ""}
                              onValueChange={(val) => setDepositReceiverIds(prev => ({ ...prev, [group.groupId]: val }))}
                            >
                              <SelectTrigger className="w-full bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700 text-xs h-9">
                                <SelectValue placeholder="Seleccionar destinatario..." />
                              </SelectTrigger>
                              <SelectContent>
                                {paymentReceivers.map((r) => (
                                  <SelectItem key={r.id} value={r.id} className="text-xs">
                                    {r.name} {r.accountInfo ? `(${r.accountInfo})` : ""} {r.isDefault ? " ⭐ (Predeterminado)" : ""}
                                  </SelectItem>
                                ))}
                              </SelectContent>
                            </Select>
                          ) : (
                            <p className="text-xs text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-900/60 p-2 rounded-md">
                              No hay destinatarios activos configurados en Balance. Podés configurarlos desde Balance &gt; Configuración.
                            </p>
                          )}
                        </div>
                      )}
