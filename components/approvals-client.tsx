  ...
  if (approvals.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-24 text-center">
        ...
        <h2 className="text-2xl font-bold text-slate-700 mb-2">¡Todo al día!</h2>
        <p className="text-slate-500">No hay reservas pendientes de aprobación.</p>
      </div>
    );
  }
          ...
          <div key={group.groupId} className={`bg-white rounded-2xl border shadow-sm overflow-hidden transition-all ${
            ...
            <div className="p-6 grid md:grid-cols-2 gap-6">
              ...
              <div className="space-y-3">
                <h4 className="text-sm font-semibold text-slate-500 uppercase tracking-wider">Datos del Huésped</h4>
                <div className="space-y-2">
                  ...
                  {primaryReservation.guestPhone && (
                    <div className="flex items-center gap-2 text-slate-700">
                      ...
                      <a href={`https://wa.me/${primaryReservation.guestPhone.replace(/\D/g, '')}`} target="_blank" className="text-green-600 hover:underline">{primaryReservation.guestPhone}</a>
                    </div>
                  )}
              ...
              {/* Booking Details */}
              <div className="space-y-3">
                <h4 className="text-sm font-semibold text-slate-500 uppercase tracking-wider">Alojamiento</h4>
                <div className="space-y-2">
                  {group.reservations.map((res) => {
                    ...
                    return (
                      <div key={res.id} className={`flex items-center gap-3 p-3 rounded-xl border ${isMySegment && isPending ? 'border-amber-200 bg-amber-50' : 'border-slate-100 bg-slate-50'}`}>
                        ...
                        <span className="text-sm font-bold text-slate-700 flex-shrink-0">${res.totalAmount.toLocaleString()}</span>
                      </div>
                    );
                  })}
                  ...
                  <div className="flex justify-between items-center pt-2 border-t border-slate-100">
                    <span className="text-sm text-slate-500 font-medium">Total (toda la estadía)</span>
            ...
            {isPending && (
              <div className="px-6 pb-6 space-y-4">
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div>
                    <h5 className="font-semibold text-slate-800 text-sm">Seña / Adelanto abonado</h5>
                    <p className="text-xs text-slate-500">Monto depositado para asegurar la reserva.</p>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-slate-500 font-bold">$</span>
                    <input 
                      type="number" 
                      className="border border-slate-300 rounded-lg px-3 py-2 w-32 focus:outline-none focus:ring-2 focus:ring-sky-500"
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  {isCombination && (
                    <p className="w-full text-xs text-slate-500 bg-blue-50 border border-blue-100 rounded-lg p-2.5 mb-1">
